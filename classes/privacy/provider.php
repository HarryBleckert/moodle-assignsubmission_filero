...
class provider implements
    ...
    public static function get_metadata(collection $collection): collection {
        $detail = [
                ...
                'filero' => 'privacy:metadata:textpurpose'
        ];
        $collection->add_database_table('assignsubmission_filero', $detail, 'privacy:metadata:tablepurpose');
    ...
    public static function delete_submission_for_context(assign_plugin_request_data $requestdata) {
        ...
        $fs = get_file_storage();
        $fs->delete_area_files($requestdata->get_context()->id, 'assignsubmission_filero',
                assignsubmission_file_FILEAREA);

        // Delete the records in the table.
        $DB->delete_records('assignsubmission_filero', ['assignment' => $requestdata->get_assignid()]);
    }
    ...
    public static function delete_submission_for_userid(assign_plugin_request_data $deletedata) {
        ...
        // Delete related files.
        $fs = get_file_storage();
        $fs->delete_area_files($deletedata->get_context()->id, 'assignsubmission_filero', assignsubmission_file_FILEAREA,
                $submissionid);

        // Delete the records in the table.
        $fileroRecs = $DB->get_records('assignsubmission_filero', ['assignment' => $deletedata->get_assignid(),
                'submission' => $submissionid]);
        foreach ($fileroRecs as $fileroRec) {
            if ($DB->get_records('assignsubmission_filero_file',
                    array('fileroid' => $fileroRec->fileroid))
            ) {
                $DB->delete_records('assignsubmission_filero_file',
                        array('fileroid' => $fileroRec->fileroid));
            }
        }
        $DB->delete_records('assignsubmission_filero', ['assignment' => $deletedata->get_assignid(),
                'submission' => $submissionid]);
    }
    ...
    public static function delete_submissions(assign_plugin_request_data $deletedata) {
        ...
        $fs->delete_area_files_select($deletedata->get_context()->id,
                'assignsubmission_filero', assignsubmission_file_FILEAREA, $sql, $params);

        $params['assignid'] = $deletedata->get_assignid();

        $fileroRecs = $DB->get_records_select('assignsubmission_filero', "assignment = :assignid AND submission $sql", $params);
        foreach ($fileroRecs as $fileroRec) {
            if ($DB->get_records('assignsubmission_filero_file',
                    array('fileroid' => $fileroRec->fileroid))
            ) {
        ...
        $DB->delete_records_select('assignsubmission_filero', "assignment = :assignid AND submission $sql", $params);
    }
}
